...
logs ( client );
let pub = "800618521817514011"; ////// kategori id
let kayıt = "800618519242604544"
let terapi = "801900394439639061"
let sorun = "800618521138429977"
let vk = "800618522690191414"
let dc = "800618522690191419"
let game = "800638297722322944"
let priv = "800618523482783755"
let alone = "800618523482783755"
...
client.on ( "message" , async message => {
    ...
    if(!['ROL ID'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.react('❌')
    var totall = (
                     await db.fetch (
...
client.on ( "message" , async msg => {
    if ( msg.content.startsWith ( ".stat" ) || msg.content.startsWith ( ".me" ) ) {
        ...
        if(!['ROL ID'].some(role => msg.member.roles.cache.get(role)) && !msg.member.hasPermission('ADMINISTRATOR')) return msg.react('❌')
        let st = member.voice
...
__Toplam__: \`${ toplam }\`
__Public Odalar__: \`${ format }\`
__Kayıt Odaları__: \`${ toplamKayıt }\`
__Sorun Çözme & Terapi__: \`${ toplamMod }\`
__Private Odalar__: \`${ toplamPriv }\`
__Alone Odalar__: \`${ toplamAlone }\`
__Oyun & Eğlence Odaları__: \`${ toplamGame }\`
───────────────
...
client.on ( "message" , async msj => {
    ...
    if(!['ROL ID'].some(role => msj.member.roles.cache.get(role)) && !msj.member.hasPermission('ADMINISTRATOR')) return msj.react('❌')
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Haise 💙 Pasifik"}, status: "idle" }); //dnd rahatsı etmeyın, online çevirimiçi, idle boşta 
});
